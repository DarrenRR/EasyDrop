...
async function mutualPickup(tripID){
    ...
    var dropoffPoint = await getPassengerStopCenter(passengers);
    var waypoint;
    if(pickupPoint === 0){
        waypoint = [{point: "none"}];
    }
    ...
    if(dropoffPoint === 0){
        waypoint.push({point: "none"});
    }
    else{
        waypoint.push({point: "center", center: dropoffPoint.lat() +","+dropoffPoint.lng()});
    }
    var driver = await getDriver(tripID);
...
async function getAvailableSeats(tripID){
    ...
    var tripPassengers;
    // Sinces routes are to be calculated for each allocation, the amount of bookings are limited to avoid waiting a very long amount of time and to avoid an over query limit by the Google Maps API.
    db.collection('Bookings').where("TripId" , "==", tripID).where("Accepted", "==", true).limit(40).get().then(snapshot => {
    ...
    setTimeout(function() {
        if(bookings.length == 0){
            ...
            bookings.forEach(function(booking){
                ...
                var emailBody = "Your booking has been allocated to " + username + "'s Trip: " + tripID + ".\n";
                emailBody += "Please check EasyDrop to view this.";
                Email.send({
                    Host: "smtp.gmail.com",
...
// ============================= Renders only the user information ================================
    ...
    async function sendRemovalEmail(row) {
        ...
        var bookingID;
        var userEmail;
        const userBooking = await db.collection('Bookings').where("TripId", "==", tripID.trim()).where("Username", "==", username.trim()).get();
        userBooking.forEach(doc => {
            userEmail = doc.data().Email;
        ...
        const res = await db.collection('Bookings').doc(bookingID).delete();
    }
